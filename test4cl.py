...
print("  Trwa RandomizedSearchCV (n_iter=20, cv=3)...\n") # ZMNIEJSZONO LICZBĘ ITERACJI

search = RandomizedSearchCV(
    estimator           = model_to_tune,
    param_distributions = param_dist,
    n_iter              = 20,         # ZMIANA: 20 zamiast 100
    cv                  = 3,
    scoring             = "f1_macro",
    random_state        = RANDOM_STATE,
    n_jobs              = -1,
...
# ─────────────────────────────────────────────────────────────────
# PODSUMOWANIE I WAŻNOŚĆ CECH (OBSŁUGA OBU MODELI)
# ─────────────────────────────────────────────────────────────────
print("=" * 65)
...
# Obsługa wyciągania cech w zależności od typu modelu
if hasattr(best_model, "feature_importances_"):
    # Dla Random Forest
    importances = best_model.feature_importances_
elif hasattr(best_model, "coef_"):
    # Dla Logistic Regression: uśredniamy wartości bezwzględne wag dla każdej klasy
    importances = np.mean(np.abs(best_model.coef_), axis=0)
    # Normalizacja do sumy równej 1 (aby wartości wyglądały podobnie jak z RF)
...
if importances is not None:
    ...
    print("  Model nie obsługuje ekstrakcji ważności cech.")
